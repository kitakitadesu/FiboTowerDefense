...
void GameBoard::loadTexture() {
    layers_.upload();
}

void GameBoard::updateScale(int screenWidth) {
    scale_ = static_cast<float>(screenWidth) / layers_.getWidth();
    layers_.setScale(scale_);
}

raylib::Vector2 GameBoard::screenToImage(raylib::Vector2 screen) const {
    return {screen.x / scale_, screen.y / scale_};
}

CellRect GameBoard::cellRect(int col, int row) const {
    return {
...
int GameBoard::hoveredCell(raylib::Vector2 mouse) const {
...
void GameBoard::drawHover(int cellIndex) const {
