                    ...
                    const GameBoard& b, bool isNight)
{
    ...
    for (auto& e : ene) if (e->getRow() == r) {
        ...
        e->draw(manTex, float(cr.w), float(cr.h));
    }
}

void Level::render(const raylib::Texture* ur3eTex, const raylib::Texture* gooseTex,
                   const raylib::Texture* solarTex, const raylib::Texture* manTex,
                   const Texture2D* nightMap, float nightAlpha) {
    const bool isNight = (nightAlpha >= 128.0f);
    // z=0: bg
    grid_.drawRange(0, 0);

    // All rows behind their building layers
    drawRow(0, ur3eTex, gooseTex, solarTex, manTex, turrets_, solarCells_, enemies_, grid_, isNight);
    drawRow(1, ur3eTex, gooseTex, solarTex, manTex, turrets_, solarCells_, enemies_, grid_, isNight);
    drawRow(2, ur3eTex, gooseTex, solarTex, manTex, turrets_, solarCells_, enemies_, grid_, isNight);
    ...
    // Row 3 — ABOVE stone
    drawRow(3, ur3eTex, gooseTex, solarTex, manTex, turrets_, solarCells_, enemies_, grid_, isNight);

    // z=3: sign, z=4: building_4
    grid_.drawRange(3, 4);

    // Row 4 — behind building_5
    drawRow(4, ur3eTex, gooseTex, solarTex, manTex, turrets_, solarCells_, enemies_, grid_, isNight);
